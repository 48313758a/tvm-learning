...
int main(){
    tvm::runtime::Module mod_dylib = tvm::runtime::Module::LoadFromFile("tvm_output_lib/cuda_yolov5s.so");
 
    // std::ifstream json_in;
    // json_in.open("./tvm_output_lib/yolov5m.json", std::ios::in);
    std::ifstream json_in("tvm_output_lib/cuda_yolov5s.json", std::ios::in);
    std::string json_data((std::istreambuf_iterator<char>(json_in)), std::istreambuf_iterator<char>());
    json_in.close();

    // parameters in binary
    std::ifstream params_in("tvm_output_lib/cuda_yolov5s.params", std::ios::binary);
    ...
    // float result[25200*85];
    float *result = new float[25200*85];
    ...
    if (!enabled) 
    {
        printf("Skip heterogeneous test because cuda is not enabled.\n");
        return 0;
    }
    ...
    TVMArrayCopyFromBytes(x, &data ,3 * 640 * 640 * sizeof(float));
    ...
    get_output(0, y);
    TVMArrayCopyToBytes(y, &result[0], 1*25200*85*sizeof(float));
